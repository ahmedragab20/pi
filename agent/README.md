...
You → pi (the lead, any model via /model or Ctrl+P)
  ...
  │           (Luna → ClinePass MiMo → vision-free)
...
├── AGENTS.md              — always-on card (routing, chore, Agent contract,
...
| `clinepass/cline-pass/kimi-k3` | ClinePass | Lead — subscription quota |
| `clinepass/cline-pass/deepseek-v4-pro` | ClinePass | Lead — subscription quota |
| `clinepass/cline-pass/qwen3.8-max` | ClinePass | Lead — subscription quota |

Everything else stays on `/model` (not the cycle): Terra, Luna, GLM, Opus `@1m`,
Fable `@1m`, MiniMax, Qwen, remaining ClinePass slugs. Worker free twins
(`opencode/deepseek-v4-flash-free`, `opencode/mimo-v2.5-free`) are
auto-provisioned outside the cycle and still used by `agents/*.md`.
