...
Normal harness `Agent` calls use agent frontmatter > explicit thinking > per-model default > global default (pi fallback medium). They do not inherit the lead's live thinking; existing or resumed worker sessions keep their level. Harness `Agent` launch paths validate levels and snapshot the selected policy for quota retries; a fallback model may clamp it. Third-party direct subagent RPC/UI paths are package-owned and not covered by this local guard. Agent frontmatter overrides rather than merely defaults explicit thinking.
